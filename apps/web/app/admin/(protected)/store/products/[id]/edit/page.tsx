...
export default async function EditStoreProductPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <><PageHeader title={`Edit ${item.name}`} description={`Draft and catalog setup for /${item.slug}. Inventory quantities are changed in the Inventory workspace.`}/><StoreProductWizard item={item} categories={categories}/></>;
}
