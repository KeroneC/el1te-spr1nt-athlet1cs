...
import { ArrowRight, CalendarDays, Handshake, Inbox, Plus, ShoppingBag, Users } from "lucide-react";
...
function Quick({href,title,description,icon:Icon=Plus}:{href:string;title:string;description:string;icon?:typeof Plus}){return <Link href={href} className="border-l-4 border-track-red bg-white p-5 shadow-sm"><Icon size={20} className="text-track-red"/><h2 className="mt-3 font-black">{title}</h2><p className="mt-1 text-sm text-slate-600">{description}</p></Link>}
