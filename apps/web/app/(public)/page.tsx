...
export default async function HomePage() {
  ...
  return <>
    ...
    <section className="achievement-band"><div className="site-container achievement-layout"><div className="achievement-image"><img src="/images/team/meet-community-static.jpg" alt="An El1te athlete smiling with two club supporters at a track meet" loading="lazy" decoding="async" /></div><div className="achievement-copy"><p className="eyebrow">Earned together</p><h2>Confidence built at the track</h2><p>Every practice, race, and finish line is a chance for young athletes to grow in discipline, teamwork, and belief.</p><Link className="text-link" href="/about">Our mission<ArrowRight size={17} aria-hidden="true" /></Link></div></div></section>
    ...
    <section className="content-section"><div className="site-container"><div className="section-heading"><div><p className="eyebrow">Season snapshots</p><h2>Moments from the team</h2></div><Link className="text-link" href="/gallery">Open gallery<ArrowRight size={17} aria-hidden="true" /></Link></div>{albums.length ? <div className="gallery-album-grid compact">{albums.map((album) => <Link href={`/gallery/${album.slug}`} key={album.slug} className="gallery-album-card">{album.coverImageUrl ? <ResponsiveMediaImage src={album.coverImageUrl} alt={album.coverAltText ?? ""} sizes="(max-width: 768px) 100vw, 33vw" /> : <div className="gallery-placeholder">Gallery album</div>}<div><p className="eyebrow">{album.imageCount} {album.imageCount === 1 ? "photo" : "photos"}</p><h2>{album.title}</h2><p>{album.description}</p></div></Link>)}</div> : <EmptyState title="Gallery albums are coming soon" message="Published team photo collections will appear here." />}</div></section>
