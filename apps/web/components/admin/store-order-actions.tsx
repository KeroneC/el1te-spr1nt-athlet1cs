...
import { LoaderCircle, Mail, Printer, RefreshCw } from "lucide-react";
...
export function StoreOrderActions({ order, superAdmin }: { order: AdminStoreOrder; superAdmin: boolean }) {
  ...
  async function mutate(path: string, body?: unknown) {
    ...
    try {
      ...
      if (redirectForAdminResponse(response)) return false;
      const result = await response.json().catch(() => ({})) as { message?: string; detail?: string; referenceId?: string };
      if (!response.ok) { setMessage(result.message ?? result.detail ?? "The order could not be updated."); setReferenceId(response.status >= 500 ? validSupportReference(result.referenceId) : null); return false; }
      router.refresh(); return true;
    } catch { setMessage("The order service could not be reached."); return false; }
