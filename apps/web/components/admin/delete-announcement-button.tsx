...
export function DeleteAnnouncementButton({ id, title }: { id: string; title: string }) {
  ...
  async function remove() {
    setSubmitting(true); setError(null); setReferenceId(null);
    try {
      ...
      if (!response.ok) { const body = await response.json() as { message?: string; referenceId?: string }; setError(body.message ?? "Announcement could not be deleted."); setReferenceId(response.status >= 500 ? validSupportReference(body.referenceId) : null); return; }
