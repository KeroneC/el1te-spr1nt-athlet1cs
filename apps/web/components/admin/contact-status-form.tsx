...
return <form onSubmit={submit} className="space-y-4 border border-slate-200 bg-white p-5"><h2 className="text-lg font-black text-track-ink">Submission status</h2><FormNotice message={mutation.message} success={mutation.success} referenceId={mutation.referenceId} /><SelectField label="Status" name="status" defaultValue={item.status} options={CONTACT_STATUSES.map(v => [v,v])} /><button type="submit" disabled={mutation.submitting} className="min-h-10 bg-track-red px-4 text-sm font-bold text-white disabled:opacity-60">{mutation.submitting ? "Saving..." : "Update status"}</button></form>; }
