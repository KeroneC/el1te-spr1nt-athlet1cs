...
export function ContentSection({ block, tone = "light" }: { block: ContentBlock; tone?: "light" | "muted" | "dark" }) {
  return <section className={`content-section tone-${tone}`}><div className={block.imageUrl ? "site-container content-media-layout" : "site-container narrow"}><div className="content-media-copy"><p className="eyebrow">{block.summary}</p><h2>{block.title}</h2><div className="prose"><p>{block.body}</p></div>{block.ctaText && block.ctaUrl && <Link className="text-link" href={block.ctaUrl}>{block.ctaText}<ArrowRight size={17} aria-hidden="true" /></Link>}</div>{block.imageUrl && <div className="content-media-image"><ResponsiveMediaImage src={block.imageUrl} alt={`Supporting image for ${block.title}`} sizes="(max-width: 768px) 100vw, 50vw" /></div>}</div></section>;
}
...
export function AnnouncementCard({ item }: { item: AnnouncementListItem }) {
  return <article className={item.isFeatured ? "content-card announcement-card featured" : "content-card announcement-card"}>{item.imageUrl && <div className="card-media"><ResponsiveMediaImage src={item.imageUrl} alt={`Club update: ${item.title}`} /></div>}<div className="card-content"><div className="card-accent" />{item.isFeatured && <span className="tag">Featured</span>}<p className="card-meta">{formatDate(item.publishDateUtc)}</p><h3><Link href={`/news/${item.slug}`}>{item.title}</Link></h3><p>{item.summary}</p><Link className="text-link" href={`/news/${item.slug}`}>Read update<ArrowRight size={16} aria-hidden="true" /></Link></div></article>;
}
