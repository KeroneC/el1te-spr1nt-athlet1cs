...
import { Clock3, PackageCheck, RefreshCw, ShoppingBag } from "lucide-react";
...
export function StoreOrderStatus() {
  ...
  const loadToken = useCallback(async (value: string) => {
    ...
    try {
      ...
      setOrder(result);
    } catch { setMessage("Order status could not be reached. Try again."); }
  ...
  async function cancel() {
    if (!order || !confirm("Cancel the complete order and request a full Square refund?")) return;
    ...
    setOrder(result);
    setMessage("The order is locked from production. Your full Square refund is being processed.");
  }
