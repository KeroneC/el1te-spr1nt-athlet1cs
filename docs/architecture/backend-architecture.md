...
Commerce follows the same boundaries. Core owns catalog, inventory, stocktake, order, reservation, webhook, and outbox data concepts. Application owns the Admin/public store, Square checkout, and outbox interfaces. Infrastructure implements `StoreAdminService`, `PublicStoreService`, the typed Square payment client, signature verifier, webhook persistence, and outbox processor. API exposes protected catalog/inventory routes, feature-gated public product routes, webhook and health routes, and hosts the always-on worker. Exact inventory and SKUs never cross the public DTO boundary.

Inventory writes use variant row versions and SQL constraints so on-hand cannot be negative or below reserved. Every successful quantity change creates an append-only `InventoryAdjustment`; a physical count additionally creates one `InventoryStocktake` and one immutable line per counted variant. Historical Square source-object IDs and import-run records remain mapped for compatibility, but no catalog-import API is exposed.
