...
- The commerce replacement is being delivered in five guarded phases. The first branch added the disabled-by-default Square foundation. The second added the private Admin catalog/inventory workspace. The third adds `/shop`, product configuration, safe stock states, deterministic previews, and a non-personal revalidated cart. The Azure demo sets `Store__PublicPreviewEnabled=true` so trusted reviewers can exercise those pages, while `Store__Enabled=false` keeps Square, payment, orders, reservations, webhooks, and commerce workers disabled. Payment, order operations, and cutover are later phases.
