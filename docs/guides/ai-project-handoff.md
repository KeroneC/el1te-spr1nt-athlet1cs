...
- The commerce replacement is being delivered in five guarded phases. The first branch added the disabled-by-default Square foundation. The second adds the private Admin catalog/inventory workspace: one-time idempotent Square draft import, categories, guided products, tracked variants, modifiers, visualizer placement, receipts, corrections, and physical stocktakes. It does not publish `/shop`, accept payments, or replace the external Square storefront.
