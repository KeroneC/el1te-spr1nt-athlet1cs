...
- The commerce replacement is being delivered in five guarded phases. The first branch added the disabled-by-default Square foundation. The second added the private Admin catalog/inventory workspace. The third adds feature-gated `/shop`, product configuration, safe stock states, deterministic previews, and a non-personal revalidated cart. `Store__Enabled=false` still keeps those routes hidden and retains the external Square storefront; payment, order operations, and cutover are later phases.
